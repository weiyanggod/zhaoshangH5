...
class PdfLoader {
    ...
    async getPDF(resolve) {
        ...
        }canvas.width = eleW * 10 // 将画布宽&&高放大两倍
        canvas.height = eleH * 10
        let context = canvas.getContext("2d")
        // context.scale(100, 100) // 增强图片清晰度
        context.translate(- eleOffsetLeft, - eleOffsetTop)
        html2canvas(ele, {
            useCORS: true, // 允许canvas画布内可以跨域请求外部链接图片, 允许跨域请求。
            dpi: window.devicePixelRatio*4, // 提升导出的文件的分辨率
            scale: 4
        }).then(async canvas => {
            ...
            let imgWidth = this.A4_WIDTH // -10为了页面有右边距
        ...
        childNodes.forEach((childDom, index) => {
            if (this.hasClass(childDom, this.splitClassName)) {
                ...
                if (this.pageNum < currentPage) {
                    ...
                    newNode.style.height = (this.pageHeight * (this.pageNum - 1) - offset2Ele + 10) + 'px'; // +30为了在换下一页时有顶部的边距
            ...
            if (this.hasClass(childDom, this.breakClassName)) {
                ...
                let remainingHeight = this.pageHeight - alreadyHeight + 25
